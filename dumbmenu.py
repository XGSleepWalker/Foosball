...
def dumbmenu(screen, menu, x_pos = 100, y_pos = 100, font = None,
	...
	fileline = f.readline()
	f.close()
	fileline = "Highscore: "+fileline[:-1]
	text = myfont.render(fileline,True, fgcolor)
	textrect = text.get_rect()
	textrect = textrect.move(60,10)
	screen.blit(text,textrect)
	pygame.display.update(textrect)
	# Draw the ">", the Cursor
	cursorpos = 0
	cursor = myfont.render(">", True, cursorcolor)
	#cursor = pygame.gfxdraw.filled_trigon(screen,0,0,0,size,int(size//2),int((1.73*size)//2), cursorcolor)
